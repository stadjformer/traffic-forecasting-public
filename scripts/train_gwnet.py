#!/usr/bin/env python
"""Train Graph-WaveNet model on traffic forecasting datasets."""
...
def train_gwnet(
    ...
    """
    Train Graph-WaveNet model on specified dataset.
    Args:
        dataset_name: Dataset name (e.g., "METR-LA", "PEMS-BAY")
        push_to_hub: Whether to push trained model to HuggingFace Hub
        hub_repo_id: HuggingFace repo ID (e.g., "username/GraphWaveNet_METR-LA")
    ...
    model_dir, auto_repo_id, is_private = utils.io.get_model_paths(
        "GWNET", dataset_name, dry_run=dry_run
    ...
    # All hyperparameters use Graph-WaveNet defaults except epochs (configurable for dry-run)
    ...
    test_predictions = utils.gwnet.get_gwnet_predictions(
        model_result, pytorch_datasets["test"]
    test_true = pytorch_datasets["test"].y.numpy()[
        ..., 0:1
    ]  # Only first feature (actual values)
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Graph-WaveNet model on traffic forecasting datasets"
    parser.add_argument(
        "dataset",
        type=str,
        ...
        "--hub-repo-id",
        default=None,
        help="HuggingFace repo ID (e.g., username/GraphWaveNet_METR-LA)",
        "--dry-run",
    ...
    train_gwnet(
        ...
        hub_repo_id=args.hub_repo_id,  # Pass through as-is, let train_gwnet handle it
